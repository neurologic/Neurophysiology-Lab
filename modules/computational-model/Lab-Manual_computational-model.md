...
You now have a computational model for $V_{in}$ in terms of $V_{ref}$, $E_{rest}$, $R_{mem}$, and the applied current ($I$). 
...
2. Calculate $V_{in}$ (relative to $V_{ref}$) when the applied current is -50 pA.
3. Draw the membrane voltage response (calculated based on your computational model) that you would predict across time in response to the following current stimulation protocol (1 sec duration, -50 pA amplitude square wave stimulation pulse applied across the membrane at t = 100 msec after the onset of a trial and followed by 100 msec of 0pA).
    :::{image} /images/sample-applied-current-neg50pA.png
    :width: 800
    :::
4. Take a look at the response of real neuron membrane to this same stimulus from the [cell types database at the Allen Brain Institute](https://celltypes.brain-map.org/experiment/electrophysiology/474626527).
...
We know that neuron membranes act as capacitors in addition to acting as a resistor. Let's add that electrical component to the model membrane circuit and update the computational model. 
...
2. What is the voltage across $C$ in terms of $V_{in}$, $V_{ref}$, and/or $E_{rest}$?
3. What is the voltage across $R$ in terms of $V_{in}$, $V_{ref}$, and/or $E_{rest}$?
4. Write the equation for the Current across a capacitor ($I_C$) in terms of the Voltage across a capacitor ($V_C$). Substitute the equation from \#2 in for $V_C$.
...
6. When $I=0$, what is the voltage across $R$ ($V_R$)?
7. When $I=0$, what is the voltage across $C$ ($V_C$)? 
8. Substitute the equations from \#4 and \#5 into the equation from \#1. Remember that we can set $V_{ref}=0$.
9. Solve the equation from \#6 for $\partial{V_{in}}$.
:::
